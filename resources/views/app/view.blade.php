...
@section('content')
    <div class="flex justify-center">
        <div class="mx-auto my-auto mt-28 md:mt-20 lg:mt-24">
            ...
            @if (session('error'))
                <div class="alert bg-red-500 rounded-xl py-3 px-2 text-white alert-dismissible mb-2">
                        ...
                        <div class="flex flex-col w-[300px] md:justify-start md:my-0 mb-2 ml-0 lg:ml-4">
                            <div class="flex flex-col gap-1 ">
                                ...
                                <div class="flex justify-between md:mt-2">
                                    <p>Harga :</p>
                                    <p class="font-semibold text-[#72c2ff] md:text-2xl">Rp.
                                        {{ number_format($product->price, 0, '.', '.'), $product->price }}</p>
                                </div>
                            ...
                            <div class="flex flex-col items-center mb-2 mt-4">
                                <form action="{{ route('transactions.store') }}" method="post">
                                    ...
                                    <button
                                        class="bg-gradient-to-r from-[#72c2ff] to-[#7277ff] text-white mt-4 px-4 py-2 rounded-md"
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 lg:gap-4 mt-4 justify-center">
                        @foreach ($productsRecommended as $product)
                            <a href="{{ route('show', ['category' => $product->category, 'id' => $product->id, 'name' => $product->name]) }}"
                                class="w-24 h-24 md:w-24 md:h-24 lg:w-32 lg:h-32">
                                <img src="{{ asset('storage/ProductCoverImages/' . $product->cover) }}"
                                    class="rounded-md w-32 h-24 lg:h-32 object-cover hover:bg-blue-100 " alt="">
                            </a>
                        @endforeach
                    </div>
                </div>
